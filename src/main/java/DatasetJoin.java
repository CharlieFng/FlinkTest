...
public class DatasetJoin {
    public static void main(String[] args) throws Exception
    {
        ...
        // OPTIMIZER_CHOOSES -- leave it to flink
        // BROADCAST_HASH_FIRST -- if first table is small
        // BROADCAST_HASH_SECOND -- if second table is small
        // REPARTITION_HASH_FIRST -- if both tables are big, but first one is smaller
        // REPARTITION_HASH_SECOND -- if both tables are big, but second one is smaller
        // REPARTITION_SORT_MERGE -- if one or both tables are already sorted
        DataSet<Tuple3<Integer, String, String>> joined = personSet.join(locationSet, JoinOperatorBase.JoinHint.BROADCAST_HASH_FIRST).where(0) .equalTo(0)
